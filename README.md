...
이름만 맞으면 입장시킵니다. 진행 기록이 갈라지지 않도록 **첫 로그인 때 입력한 학번이
그 학생의 고정 ID로 저장**되며, 이후 다른 학번을 입력해도 막지 않고 자동으로 기존
기록에 이어집니다. (고정 ID를 바꾸고 싶으면 교사가 Firestore 콘솔에서 해당 `roster`
문서의 `studentId` 필드를 지우면 됩니다.)
