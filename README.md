...
학생은 비밀번호 없이 **5자리 학번 + 이름**으로 입장합니다. 내부적으로는 서버가 사전
등록 명단(`roster`)과 대조한 뒤, 학번을 uid로 하는 Firebase 커스텀 토큰을 발급해
Firestore 보안 규칙이 "본인 데이터만 쓰기"를 검증할 수 있게 합니다. 교사는 PIN으로
로그인하며 `teacher: true` 클레임이 담긴 토큰을 받습니다.
