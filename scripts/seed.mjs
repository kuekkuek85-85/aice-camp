...
const DAY2 = {
  ...
  steps: [
    ...
    },
    {
      ...
      links: [{ label: "AI 코디니 실습 사이트(codex) 열기", url: CODEX_URL }],
      hints: [
        "소수는 1과 자기 자신으로만 나누어지는 수예요. 2부터 (수-1)까지 하나씩 나눠보며 나누어떨어지는 수가 있는지 확인해보세요 — 반복문과 나머지 연산이 필요해요!",
        "반복 변수 i를 2부터 (수-1)까지 늘리면서, (수를 i로 나눈 나머지) = 0 인 순간이 한 번이라도 있으면 소수가 아니에요 — '약수 있음' 변수를 만들어 기록해두세요. (주의: 1은 소수가 아니에요!)",
        "1일차의 호출어 블록을 기억하나요? \"지니야\" 호출어가 인식되면 그 안에서 소수 판별 함수를 호출하도록 연결하세요 — 부르면 대답하는 나만의 수학 비서 완성!",
      ],
    },
  ],
};
